...
This repository contains the source code and content for my personal portfolio website: a small static site built around research, projects, writing, and a CV-style experience timeline.

The site started from Devin Silvia's [Professional Portfolio Template](https://github.com/devinsilvia/professional_portfolio_template), but it has been heavily adapted into a multi-page personal website with a self-hosted writing system, a scholarship/research page, richer media support, and a filterable CV timeline.

## Overview

The site is intentionally source-driven. Most content lives in YAML files, the page layouts live in Jinja templates, and `build.py` turns everything into static HTML under `dist/`.

At a high level, the website has five main public surfaces:
...
The goal is not to be a general-purpose site generator anymore. It is now mostly a reproducible workflow for maintaining my own website without manually editing generated HTML.
...
Generated and local-only folders such as `dist/`, `.venv/`, `__pycache__/`, `TODO.md`, and `content/drafts/` are intentionally ignored by git.
...
`dist/` is generated by the build process and ignored by git. It is the local preview and deployment artifact, not the source of truth.
...
The original template was released under the MIT License. The license notice is preserved in [`LICENSE`](LICENSE).

## Note About AI Assistance

The original template noted that it was drafted and built with the help of AI, specifically Claude.
